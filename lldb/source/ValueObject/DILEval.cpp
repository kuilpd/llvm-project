...
GetPtrDiffType(std::shared_ptr<ExecutionContextScope> ctx) {
  ...
  lldb::StackFrameSP stackframe_sp = ctx->CalculateStackFrame();
  llvm::Expected<lldb::TypeSystemSP> type_system =
      DILGetTypeSystemFromCU(stackframe_sp);
  if (!type_system)
    return type_system.takeError();
...
Interpreter::EvaluateBinaryShift(BinaryOpKind kind, lldb::ValueObjectSP lhs,
  ...
  return EvaluateScalarOp(kind, lhs, rhs, lhs->GetCompilerType(), location);
}
...
Interpreter::EvaluateLogical(const BinaryOpNode *node) {
  ...
  bool break_early =
      (node->GetKind() == BinaryOpKind::LAnd) ? !lhs_val : lhs_val;
  if (break_early)
    return ValueObject::CreateValueObjectFromBool(m_target, lhs_val, "result");
...
Interpreter::Visit(const ConditionalNode *node) {
  ...
  if (value_or_err) {
    if (*value_or_err) {
      auto true_or_err = Evaluate(node->GetTrueOperand());
      if (!true_or_err)
        return true_or_err;
      return *true_or_err;
    }
    auto false_or_err = Evaluate(node->GetFalseOperand());
    if (!false_or_err)
      return false_or_err;
    return *false_or_err;
  }
...
Interpreter::Visit(const MethodCallNode *node) {
  ...
  auto obj_or_err = EvaluateAndDereference(node->GetObject());
  if (!obj_or_err)
    return obj_or_err;
