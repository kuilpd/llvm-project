...
const uint32_t kWaitForEventTimeout = 15;
#endif
...
lldb::SBProcess LaunchTestProgram(lldb::SBDebugger debugger,
  ...
  if (break_line != 0) {
    auto source_file = filename_of_source_path(name);
    bp = target.BreakpointCreateByLocation(source_file.c_str(), break_line);
  } else
    bp = target.BreakpointCreateByName(name.c_str());
